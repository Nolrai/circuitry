...
import Diagrams.Prelude hiding (showTrace, trace)
...
runCircuit (Circuit dsl) =
  let c = execStateT dsl def
...
findPort ::
  ...
  DiaID s ->
  nm ->
  C.Constrained s b n m (P2 (C.Expr s n))
findPort d name = C.newPointOn d (location . fromJust . lookupName name)

leftOf ::
  (Hashable n, Semigroup m, RealFrac n, Floating n, Monoid m) =>
  P2 (C.Expr s n) ->
  P2 (C.Expr s n) ->
  Circuit s b n m ()
